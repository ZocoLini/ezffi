...
impl FFINamer {
    pub fn name_struct(rust_ty_name: &Ident) -> Ident {
        ...
        let sufix = crate::CONFIG.type_sufix();

        let ffi_ty_name = format!("{}{}{}", prefix, rust_ty_name, sufix);
    ...
    pub fn name_fn(rust_fn_name: &Ident, impl_ty: Option<&Type>) -> Ident {
        ...
        let sufix = crate::CONFIG.fns_sufix();
        ...
        let ffi_fn_name = if let Some(ty) = impl_ty {
            ...
            let ffi_fn_name = format!("{}{}_{}{}", prefix, ffi_ty_name, rust_fn_name, sufix);
            crate::CONFIG.fns_case_style().format(&ffi_fn_name)
        } else {
            let ffi_fn_name = format!("{}{}{}", prefix, rust_fn_name, sufix);
            crate::CONFIG.fns_case_style().format(&ffi_fn_name)
        };
    ...
    pub fn name_free_fn(rust_ty_name: &Ident) -> Ident {
        ...
        let sufix = crate::CONFIG.free_fns_sufix();

        let ffi_ty_name = Self::name_struct(rust_ty_name);

        let ffi_free_fn_name = format!("{}{}_free{}", prefix, ffi_ty_name, sufix);
